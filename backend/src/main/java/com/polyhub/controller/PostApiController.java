...
// } catch (Exception e) {
// e.printStackTrace();
// return ResponseEntity.badRequest().body("Đã xảy ra lỗi khi đăng bài: " +
// e.getMessage());
// }
// }
...
// public ResponseEntity<?> sharePost(@PathVariable Long postId, @RequestBody
// Map<String, String> requestBody, Principal principal) {
// if (principal == null) {
// return ResponseEntity.status(401).body("Vui lòng đăng nhập để thực hiện chức
// năng này.");
// }
...
// // Trả về một ID để test, tránh bóc tách Entity trực tiếp gây lỗi Jackson Đệ
// quy
...
// @PutMapping("/{postId}")
// public ResponseEntity<?> updatePost(@PathVariable Long postId, @RequestBody
// Map<String, String> payload, Principal principal) {
// if (principal == null) return ResponseEntity.status(401).body("Chưa đăng
// nhập");
// try {
...
// public ResponseEntity<?> deletePost(@PathVariable Long postId, Principal
// principal) {
// if (principal == null) return ResponseEntity.status(401).body("Chưa đăng
// nhập");
// try {
...
// public ResponseEntity<?> togglePrivacy(@PathVariable Long postId, Principal
// principal) {
// if (principal == null) return ResponseEntity.status(401).body("Chưa đăng
// nhập");
// try {
// Post updatedPost = postService.togglePrivacy(postId, principal.getName());
// String status = updatedPost.getIsPrivate() ? "riêng tư (Chỉ mình tôi)" :
// "công khai";
// return ResponseEntity.ok(Map.of("message", "Đã đổi bài viết sang chế độ " +
// status, "isPrivate", updatedPost.getIsPrivate()));
// } catch (Exception e) {
// return ResponseEntity.badRequest().body(e.getMessage());
// }
// }
...
// public ResponseEntity<?> reportPost(@PathVariable Long postId, @RequestBody
// Map<String, String> payload, Principal principal) {
// if (principal == null) return ResponseEntity.status(401).body("Chưa đăng
// nhập");
// try {
...
// return ResponseEntity.ok(Map.of("message", "Cảm ơn bạn đã báo cáo. Quản trị
// viên sẽ xử lý sớm."));
...
// // --- API Toggle Like bài viết ---
// @PostMapping("/{postId}/like")
// public ResponseEntity<?> toggleLike(@PathVariable Long postId, Principal
// principal) {
// if (principal == null) {
// return ResponseEntity.status(401).body("Vui lòng đăng nhập để thích bài
// viết.");
// }
// try {
// String username = principal.getName();
// boolean isLiked;
// java.util.Optional<Like> existingLike =
...
// // --- API Lấy số lượng like và trạng thái like của bài viết ---
// @GetMapping("/{postId}/like")
// public ResponseEntity<?> getLikeStatus(@PathVariable Long postId, Principal
// principal) {
// try {
// long likesCount = likeRepository.countByPostId(postId);
// boolean isLiked = false;
// if (principal != null) {
// isLiked = likeRepository.findByPostIdAndUsername(postId,
// principal.getName()).isPresent();
// }
// Map<String, Object> response = new HashMap<>();
// response.put("likesCount", likesCount);
// response.put("isLiked", isLiked);
